...
const ShaderCanvas = () => {
  ...
  useEffect(() => {
    // Force dark background to match the site's aesthetics
  ...
  useEffect(() => {
    ...
    const render = (time: number) => {
      ...
      // Update resolution uniform using client dimensions
  ...
  return <canvas ref={canvasRef} className="absolute inset-0 w-full h-full block z-0 bg-transparent pointer-events-none" />;
};
...
export const PricingCard = ({
  ...
  const cardClasses = `
    ...
    from-white/10 to-white/5 border border-white/10 backdrop-brightness-[0.91] z-10 text-white cursor-pointer
    ${isPopular 
      ? 'scale-105 hover:scale-110 relative ring-2 ring-cyan-400/20 from-white/20 to-white/10 border-cyan-400/30 shadow-2xl hover:shadow-cyan-500/20' 
      : 'hover:scale-105 hover:shadow-2xl hover:bg-white/10 hover:border-white/20'
    }
  `;
  const buttonClasses = `
  ...
  return (
    <div className={cardClasses.trim()}>
      ...
      <div className="mb-3 text-white">
        <h2 className="text-[48px] font-extralight tracking-[-0.03em] font-display">{planName}</h2>
        <p className="text-[16px] text-white/70 mt-1 font-sans">{description}</p>
      </div>
      <div className="my-6 flex items-baseline gap-2 text-white">
        <span className="text-[48px] font-extralight font-display">${price}</span>
      ...
      <ul className="flex flex-col gap-2 text-[14px] text-white/90 mb-6 font-sans">
        {features.map((feature, index) => (
          <li key={index} className="flex items-center gap-2 text-left">
            <CheckIcon className="text-cyan-400 min-w-[16px]" /> {feature}
          </li>
        ))}
      </ul>
...
export const ModernPricingPage = ({
  ...
  return (
    <div className="relative text-foreground w-full overflow-hidden py-24 md:py-32 rounded-3xl">
      {showAnimatedBackground && <ShaderCanvas />}
      <main className="relative w-full flex flex-col items-center justify-center px-4">
        <div className="w-full max-w-5xl mx-auto text-center mb-14 z-10">
        ...
        <div className="flex flex-col md:flex-row gap-8 md:gap-6 justify-center items-stretch w-full max-w-5xl z-10">
