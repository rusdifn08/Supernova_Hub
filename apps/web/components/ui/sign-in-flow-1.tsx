...
function MiniNavbar() {
  ...
  const loginButtonElement = (
    <Link href="/" className="px-4 py-2 sm:px-3 text-xs sm:text-sm border border-[#333] bg-[rgba(31,31,31,0.62)] text-gray-300 rounded-full hover:border-white/50 hover:text-white transition-colors duration-200 w-full sm:w-auto text-center block">
      Back
    </Link>
  );
...
export const SignInPage = ({ className }: SignInPageProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log(`[AUTH CLIENT] Target Endpoint: ${endpoint}`);
      console.log(`[AUTH CLIENT] Payload:`, { ...body, password: "***" });
      ...
      console.log(`[AUTH CLIENT] Response Status: ${res.status} ${res.statusText}`);
      ...
      // trigger reverse animation to success screen
      ...
      console.error(`[AUTH CLIENT] Network or execution error:`, err);
      setErrorMsg(err.message || "An error occurred connecting to the backend. Is it running?");
    }
  };
                  ...
                  >
                    <div className="space-y-1">
                      ...
                      <p className="text-[1.2rem] text-white/70 font-light">
                        {isSignUp ? "Join our interactive learning platform" : "Sign in to continue your journey"}
                      </p>
                    </div>
                    
                    <div className="space-y-4">
                      {errorMsg && (
                        <div className="bg-red-500/10 border border-red-500/50 text-red-500 text-sm px-4 py-2 rounded-xl text-left">
                          {errorMsg}
                        </div>
                      )}
                      <form onSubmit={handleSubmit} className="space-y-3">
                        {isSignUp && (
                          <input 
                            ...
                            className="w-full backdrop-blur-[1px] text-white border border-white/10 bg-transparent rounded-xl py-3 px-4 focus:outline-none focus:border-white/30 text-left"
                            required
                          />
                        )}
                        <input 
                        ...
                        <button 
                          type="submit"
                          className="w-full text-black font-semibold rounded-xl bg-white hover:bg-gray-200 transition-colors py-3 mt-4"
                        >
                    ...
                    <p className="text-xs text-white/40 pt-10">
                      By continuing, you agree to the <Link href="#" className="underline text-white/40 hover:text-white/60 transition-colors">Terms</Link> and <Link href="#" className="underline text-white/40 hover:text-white/60 transition-colors">Privacy Policy</Link>.
                    </p>
                  </motion.div>
                ) : (
                  <motion.div 
                  ...
                  >
                    ...
                    <Link href="/dashboard" passHref>
                      <motion.button 
                        ...
                        animate={{ opacity: 1 }}
                        transition={{ delay: 1 }}
                        className="w-full rounded-xl bg-white text-black font-medium py-3 hover:bg-gray-200 transition-colors"
                      >
